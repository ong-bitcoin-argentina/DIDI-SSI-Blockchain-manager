...
describe("BlockchainManager did getters", () => {
  ...
  describe("getDidContract", () => {
    test("Get Lacchain ethr-did-registry address", async() => {
      ...
      const resul = await BlockchainManager.getDidContract(options,Constants.BLOCKCHAIN.PROVIDER_CONFIG.networks[5].registry,web3);
      expect(resul._address).toBe(Constants.BLOCKCHAIN.PROVIDER_CONFIG.networks[5].registry);
    });
