 ...
 describe("compareDid ", () => {
   // did:ethr:net1:0x123 != did:ethr:net2:0x123
    test("Compare with same did and diferent blockchain", async() => {
      const result = await BlockchainManager.compareDid(DIDS.didWithNwtwork, DIDS.otherDidOtherNetwork);
      expect(result).toBe(false);
    });

    //did:ethr:net:0x123 != did:ethr:net:0x124
    test("Compare with diferent did and same blockchain", async() => {
        const result = await BlockchainManager.compareDid(DIDS.didWithNwtwork, DIDS.otherDidSameNetwork);
        expect(result).toBe(false);
      });
      ...
      test("Compare with same did, one with blockchain and one without blockchain", async() => {
        const result = await BlockchainManager.compareDid(DIDS.didWithNwtwork, DIDS.didWithoutNetwork);
        expect(result).toBe(true);
      });

      //did:ethr:net:0x123 == did:ethr:net:0x123
      test("Compare with same did and same blockchain", async() => {
        const result = await BlockchainManager.compareDid(DIDS.didWithNwtwork, DIDS.didWithNwtwork);
        expect(result).toBe(true);
      });
  });